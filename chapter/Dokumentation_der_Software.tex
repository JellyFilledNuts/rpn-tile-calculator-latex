...
\subsection{Dokumentation der Paketstruktur des Android-Projektes [Schwenke]}
...
\quad \quad \textbf{ Eingabe:} \code{OPolynom}: x\^{}2 – 8x + 17, untere Grenze: 2, obere Grenze 5

\quad \quad \textbf{Erwartetes Ergebnis:} 6

\code{getAntiderivative()}

\quad \quad \textbf{Eingabe:} \code{OPolynom}: -3,21x\^{}3 + 3x + 7

\quad \quad \textbf{Erwartetes Ergebnis}: \code{OPolynom}: -0,8025x\^{}4 + 1,5x\^{}2 + 7x + c
...
\begin{figure}[bht]
	...
	} catch (SeveralExceptions e) { // Simplified for readability
